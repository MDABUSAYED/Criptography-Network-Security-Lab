...
def test(input=""):
    ...
    infile=open('3.3_query.txt', 'r')
    query=infile.read()
    infile.close()

    infile=open('3.3_command3.txt', 'r')
    extension=infile.read()
    infile.close()    
    ...
    url2="token="
    url1="token="+h.digest().encode("hex")+key+m
    h = md5(state=h.digest(),count=512)
    h.update(extension)
    print h.digest().encode("hex")
    padding=chr(128)+chr(0)*(56-len(key)-len(m)-1)+struct.pack('<q',len(m+key)*8)
    mess=key+m+padding+extension
    
    
    url2=url2+h.digest().encode("hex")+"&"
    url2=url2+key+m+padding.encode("hex")+extension
    print md5(mess).digest().encode("hex")
    
    infile=open('solution33.txt', 'w')
    infile.write("Given  Url   : "+url1+"\n\n")
    infile.write("Modified Url : "+url2+"\n\n")
    infile.write("Secret key   : "+key+"\n\n")
    infile.write("Message      : "+m+"\n\n")
    infile.write("Extension    : "+extension+"\n\n")
    infile.close()
